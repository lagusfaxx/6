...
export default function HomeMoteles() {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ entityType: "establishment", categorySlug: "motel", sort: "near", limit: "8" });
    if (location) {
    ...
    apiFetch<{ results: any[]; total: number }>(
      ...
      .then((res) => setMoteles(res?.results ?? []))
      .catch(() => {});
    return () => { controller.abort(); };
  }, [location]);

  if (!moteles.length) return null;
