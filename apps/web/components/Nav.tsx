...
const navItems = [
  ...
  { href: "/explore", label: "Cerca tuyo", icon: Briefcase, protected: false },
...
export default function Nav() {
  ...
  const isMotelProfile =
    ptype === "ESTABLISHMENT" || role === "MOTEL" || role === "MOTEL_OWNER";

  const dynamicItems = isMotelProfile
    ? [
        {
          href: "/dashboard/motel",
          label: "Dashboard",
          icon: Hotel,
          protected: true,
        },
  ...
  return (
    <>
      ...
      >
        <div
          className="mx-auto grid max-w-[520px] px-3 py-2"
          style={{
            gridTemplateColumns: `repeat(${dynamicItems.length}, minmax(0, 1fr))`,
          }}
        >
          {dynamicItems.map((item) => {
            const active =
