...
import { X, MapPin, ChevronLeft, ChevronRight, MessageCircle, Eye, Tag, Briefcase, Loader2, Sparkles, Hotel, ShoppingBag } from "lucide-react";
...
type Props = {
  profile: {
    ...
    profileType?: "PROFESSIONAL" | "ESTABLISHMENT" | "SHOP" | "CREATOR" | null;
  };
  onClose: () => void;
};
...
export default function ProfilePreviewModal({ profile, onClose }: Props) {
  ...
  const pType = profile.profileType;
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-black/80 p-0 sm:p-4 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        {/* CTAs — Contextual by profile type */}
          ...
          {isEstablishment && (
            <div className="flex gap-2">
              <Link
                href={profileHref}
                onClick={onClose}
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 py-3.5 text-sm font-bold transition hover:brightness-110 shadow-[0_8px_24px_rgba(245,158,11,0.3)]"
              >
              ...
              <Link
                href={profileHref}
                onClick={onClose}
                className="flex items-center justify-center gap-1.5 rounded-xl border border-white/15 bg-white/[0.06] px-4 py-3.5 text-sm font-medium text-white/80 transition hover:bg-white/[0.1]"
              >
                <Eye className="h-4 w-4" />
                Ver perfil
              </Link>
          ...
          {isShop && (
            <div className="flex gap-2">
              <Link
                href={profileHref}
                onClick={onClose}
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-rose-500 to-pink-500 py-3.5 text-sm font-bold transition hover:brightness-110 shadow-[0_8px_24px_rgba(244,63,94,0.3)]"
              >
