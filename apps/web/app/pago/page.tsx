...
import {
  ...
  Copy, Check, AlertCircle, Shield, Zap, Calendar,
} from "lucide-react";
...
type Tab = "flow" | "transfer";

export default function PagoPage() {
  const router = useRouter();
  const { me } = useMe();
  const { status: sub } = useSubscriptionStatus();
    ...
    <div className="mx-auto max-w-lg px-4 py-6 pb-24">
      ...
      {tab === "flow" && (
        <div className="rounded-3xl border border-white/[0.07] bg-white/[0.02] p-5 space-y-4">
          <div>
            <h2 className="text-sm font-semibold mb-1">Pago con Flow.cl</h2>
            <p className="text-xs text-white/50">
              Paga de forma segura con tarjeta de crédito, débito o transferencia bancaria a través de Flow.
            </p>
          </div>
