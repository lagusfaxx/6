...
export default function ServicesPage() {
  ...
  return (
    <div className="min-h-[100dvh] text-white">
      {/* Hero header */}
      <section className="relative overflow-hidden border-b border-white/[0.06] bg-gradient-to-b from-[#0e0e12] to-transparent px-4 pb-6 pt-8">
              ...
              {items.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-[5/4] sm:aspect-[16/10] overflow-hidden bg-gradient-to-br from-white/5 to-transparent">
                      {img ? (
                        <img
                          src={img}
                          alt={s.title}
                      ...
                      <div className="absolute left-3 top-3">
                        <span className={`rounded-full border px-2.5 py-1 text-[10px] font-medium backdrop-blur-xl ${
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4">
                        <h3 className="text-sm font-semibold leading-tight">{s.title || "Servicio"}</h3>
                      </div>
                    </div>
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-sm font-medium">{s.owner?.displayName || s.owner?.username || "Profesional"}</div>
                          <div className="flex items-center gap-2 text-xs text-white/40">
                            {s.category && <span>{s.category}</span>}
                            {s.owner?.city && <span>· {s.owner.city}</span>}
                          </div>
                        </div>
                        {s.price != null && s.price > 0 && (
                          <div className="text-right">
                            <div className="text-sm font-bold">${s.price.toLocaleString("es-CL")}</div>
                          </div>
                        )}
                      </div>
                    </div>
