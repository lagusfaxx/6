...
type PhoneChangeState = {
  phone: string | null;
  locked: boolean;
  request: PhoneChangeRequest | null;
};

/**
 * Teléfono de contacto. Para una profesional con número ya fijado el campo
 * queda bloqueado: cambiarlo por su cuenta deja el anuncio anterior con un
 * número que no responde, así que el cambio se pide y lo revisa el equipo.
 * Mientras no esté bloqueado (cuentas nuevas, otros tipos de perfil) se edita
...
export default function PhoneField({
  ...
  async function load() {
    try {
      setState(await apiFetch<PhoneChangeState>("/profile/phone-change"));
    } catch {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      await apiFetch("/profile/phone-change", {
        method: "POST",
        body: JSON.stringify({ phone: newPhone.trim(), reason: reason.trim() || null }),
      });
      setModalOpen(false);
      setNewPhone("");
      setReason("");
  ...
  async function cancel() {
    ...
    setSending(true);
    try {
      await apiFetch(`/profile/phone-change/${pending.id}/cancel`, { method: "POST" });
  ...
  return (
    <div className="grid gap-1.5">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
        <span className="flex-1 text-sm text-white/80">{state?.phone || value}</span>
        <button
          type="button"
          onClick={() => {
            setModalOpen(true);
      ...
      {pending ? (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-amber-500/20 bg-amber-500/[0.07] px-3 py-2 text-[11px] text-amber-200">
          ...
          <span>
            En revisión: <strong>{pending.requestedPhone}</strong>
          </span>
          <button
            type="button"
            onClick={cancel}
        ...
        <p className="rounded-xl border border-red-500/20 bg-red-500/[0.07] px-3 py-2 text-[11px] text-red-200">
          Tu última solicitud ({rejected.requestedPhone}) fue rechazada
          {rejected.adminNote ? `: ${rejected.adminNote}` : "."}
        </p>
      ) : (
        <span className="text-[11px] text-white/30">
          Tu número es la vía de contacto de tu anuncio, por eso el cambio lo revisa el equipo.
        </span>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-4 backdrop-blur-sm sm:items-center">
          <div className="w-full max-w-sm rounded-3xl border border-white/10 bg-[#0e0b16] p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-sm font-bold text-white">Solicitar cambio de número</h3>
            ...
            <button
              type="button"
              onClick={submit}
              disabled={sending || newPhone.trim().length < 8}
