    ...
    <div className="mx-auto w-full max-w-2xl space-y-5 pb-10">
      {loading ? (
        ...
        <motion.div initial="hidden" animate="visible" className="space-y-5">
          ...
          {/* Two-column grid for subscription + sidebar info */}
          {(requiresPayment || isProfessional || links.length > 0) && (
            <motion.div custom={2} variants={fadeUp} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {isTrialPeriod ? (
                        <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 px-4 py-3 space-y-2">
                          ...
                          <p className="text-xs text-white/50">
                            Vence en <span className="font-semibold text-amber-400">{subscriptionStatus.daysRemaining || 0} días</span>.
                          </p>
                        </div>
                      ) : subscriptionStatus.isActive ? (
                        <>
              ...
              {/* Right column: Visibility + Referral + Management */}
              <div className="space-y-4">
                {/* Visibility tip for professionals */}
                {isProfessional && (
                  <div className="rounded-3xl border border-fuchsia-500/15 bg-gradient-to-br from-fuchsia-600/[0.06] to-transparent p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-white/50">
                          Sube stories, completa tu perfil y activa UMate para monetizar contenido exclusivo.
                        </p>
                ...
                {/* Management links */}
                {links.length > 0 && (
                  <div className="rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur-2xl shadow-[0_12px_40px_rgba(0,0,0,0.2)] p-5">
                    ...
                    <div className="space-y-1">
                      {links.map((link) => (
                        ...
                        >
                          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/5 text-white/60">
                            {link.icon}
                          </div>
          ...
          <motion.div custom={3} variants={fadeUp} className="flex flex-col sm:flex-row gap-3">
            {isAdmin && (
              <Link
                href="/admin"
                className="flex flex-1 items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 hover:bg-white/[0.06] transition"
              >
                <Shield className="h-5 w-5 text-amber-400" />
                <div>
                  <div className="text-sm font-medium">Admin</div>
                  <div className="text-xs text-white/40">Panel de administración</div>
                </div>
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 rounded-xl border border-white/[0.06] bg-white/[0.02] px-5 py-3 text-sm text-white/40 transition-all hover:border-red-500/20 hover:bg-red-500/[0.04] hover:text-red-400 ${isAdmin ? "flex-1" : "w-full"}`}
            >
