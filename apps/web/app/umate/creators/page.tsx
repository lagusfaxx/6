...
export default function CreatorsPage() {
  ...
  return (
    <div className="space-y-5 pb-10">
      <div>
        <h1 className="text-2xl font-black text-slate-900">Creadoras</h1>
        <p className="text-sm text-slate-500">Encuentra perfiles por estilo, energía y tipo de contenido.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {creators.map((c) => (
          <Link key={c.id} href={`/umate/profile/${c.user.username}`} className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm transition hover:shadow-md">
            <div className="h-32 bg-gradient-to-br from-fuchsia-100 to-orange-100">{c.coverUrl && <img src={c.coverUrl} alt="" className="h-full w-full object-cover" />}</div>
            <div className="-mt-8 px-4 pb-4">
              ...
              <p className="mt-2 font-bold text-slate-900">{c.displayName}</p>
