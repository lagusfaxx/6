...
async function updateProfile(req: any, res: any) {
  ...
  const user = await prisma.user.update({
    ...
    data: {
      ...
      servicesTags: Array.isArray(servicesTags) ? servicesTags.map((x) => String(x)).filter(Boolean) : (typeof servicesTags === "string" ? String(servicesTags).split(",").map((x) => x.trim()).filter(Boolean) : undefined),
