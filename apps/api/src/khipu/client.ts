...
// ── Flow Plans/Customer/Subscription API ────────────────────────────

export function signFlowParams(params: Record<string, string>): string {
  // Flow: sort keys ASC, concatenate key+value (SIN url-encode), then HMAC-SHA256
...
async function flowFetch<T>(
  ...
  if (method === "GET") {
    ...
    // ✅ Flow espera x-www-form-urlencoded, NO multipart/form-data
...
export async function createFlowPlan(req: FlowCreatePlanRequest): Promise<FlowPlan> {
  return flowFetch<FlowPlan>("/plans/create", "POST", toStringRecord(req));
}
