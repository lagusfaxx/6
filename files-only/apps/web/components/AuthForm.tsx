...
export default function AuthForm({
  ...
  useEffect(() => {
    ...
    apiFetch<{
      ...
      .then((res) => setActiveTerms(res?.terms || null))
      .catch(() => setActiveTerms(null));
  }, [mode]);

  const termsDownloadUrl = useMemo(() => {
    if (activeTerms?.pdfUrl) return activeTerms.pdfUrl;
    if (activeTerms?.contentUrl) return activeTerms.contentUrl;
    return profileType === "CLIENT"
      ? "/terms/cliente-terminos.pdf"
      : "/terms/profesional-terminos.pdf";
  }, [activeTerms, profileType]);
      ...
      if (mode === "register") {
        ...
        const res = await apiFetch("/auth/register", {
          ...
          body: JSON.stringify({
            ...
            acceptTerms,
            acceptedVersion: activeTerms?.version || "legacy",
        ...
        <div className="grid gap-2">
          <label className="flex items-start gap-3 text-xs text-white/60">
            ...
            <span>
              ...
              {activeTerms?.version ? ` (${activeTerms.version})` : ""}.
            </span>
          </label>
          <a
          ...
          >
            Descargar términos (
            {profileType === "CLIENT" ? "cliente" : "profesional/comercio"})
          </a>
        </div>
      ) : null}
