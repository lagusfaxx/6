...
export default function ProfileDetailView({
  ...
  useEffect(() => {
    ...
    apiFetch<{ average: number; total: number }>(
      `/profiles/${professional.id}/reviews/summary`,
    )
      .then((res) =>
        setReviewSummary({ average: res.average || 0, total: res.total || 0 }),
      )
      .catch(() => setReviewSummary({ average: 0, total: 0 }));
    ...
    apiFetch<{ reviews: Review[] }>(
      `/profiles/${professional.id}/reviews?page=${reviewsPage}&limit=6`,
    )
      .then((res) => setReviews(res.reviews || []))
      .catch(() => setReviews([]));
  }, [me?.user?.id, professional, reviewsPage]);

  const canReview = (me?.user?.profileType || "").toUpperCase() === "CLIENT";

  const submitReview = async () => {
    if (!professional || !commentInput.trim()) return;
    ...
    setCommentInput("");
    setReviewsPage(1);
    const summary = await apiFetch<{ average: number; total: number }>(
      `/profiles/${professional.id}/reviews/summary`,
    );
    setReviewSummary({
    ...
    if (me?.user?.id) {
      const full = await apiFetch<{ reviews: Review[] }>(
        `/profiles/${professional.id}/reviews?page=1&limit=6`,
      );
      setReviews(full.reviews || []);
    }
  };
      ...
      <section className="relative w-full overflow-hidden">
        <div className="relative aspect-[5/4] w-full overflow-hidden md:aspect-[16/6]">
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4 md:p-6">
            <span
              className={`rounded-full border px-3 py-1 text-xs font-medium backdrop-blur-md ${availabilityState.className}`}
            >
        ...
        <div className="min-w-0 space-y-4 md:space-y-5">
          ...
          <section className="min-w-0 rounded-2xl bg-white/[0.03] p-4 md:rounded-3xl md:p-6">
            ...
            <p className="mt-1 text-sm text-white/70">
              Promedio: {reviewSummary.average.toFixed(1)} ★ ·{" "}
              {reviewSummary.total} reseñas
            </p>

            {!me?.user?.id ? (
              <p className="mt-3 text-xs text-white/55">
                Inicia sesión para ver comentarios completos.
              </p>
            ) : (
            ...
            {canReview ? (
              <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3">
                <p className="text-xs text-white/60">Deja tu review</p>
                <select
                  className="mt-2 rounded-lg border border-white/15 bg-transparent px-2 py-1 text-sm"
                  value={ratingInput}
                ...
                <textarea
                  ...
                  className="mt-2 w-full rounded-lg border border-white/15 bg-transparent p-2 text-sm"
                  rows={3}
                ...
                <button
                  ...
                  className="mt-2 rounded-lg border border-fuchsia-400/40 bg-fuchsia-500/20 px-3 py-1.5 text-xs"
                >
                  Guardar review
                </button>
              </div>
        ...
        <aside className="hidden min-w-0 md:block">
          <div className="sticky top-[88px] min-w-0 rounded-3xl border border-white/10 bg-[linear-gradient(180deg,rgba(36,18,49,0.92)_0%,rgba(20,11,34,0.96)_100%)] p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <span
                className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${availabilityState.className}`}
              >
                {availabilityState.label}
              </span>
            ...
            <div className="mt-4 space-y-2.5">
              <button
                ...
                className="btn-primary w-full rounded-2xl py-3 text-sm"
              >
                Enviar mensaje
              </button>
              <button
                onClick={() => handleChatClick("request")}
                className="btn-secondary w-full rounded-2xl py-3 text-sm"
              >
                Solicitar servicio
              </button>
              <button
