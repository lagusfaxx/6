...
const PUBLIC_PREFIXES = [
  ...
  "/reviews", // reviews summary/public endpoints
  "/terms", // términos activos
  "/stories", // stories públicas
  "/ads", // ads públicos
  "/attributes", // filtros públicos
  "/explore", // explore public data
...
function isSubscriptionProtected(pathname: string) {
  return SUBSCRIPTION_PROTECTED_PREFIXES.some((p) => pathname.startsWith(p));
}
...
export async function requireAuth(
  ...
  try {
    ...
    if (req.method === "GET") {
      const p = req.path;
      const isPublicServicesList =
        p === "/services" ||
        p === "/services/global" ||
        /^\/services\/[^/]+\/items$/.test(p);
    ...
    if (isSubscriptionProtected(req.path) && !isBusinessPlanActive(user)) {
      ...
      return res.status(403).json({
        ...
        message:
          "Tu periodo de prueba ha expirado. Por favor, actualiza tu suscripción para continuar usando la app.",
