    ...
    const enriched = profiles
      ...
      .map((p) => {
        ...
        return {
          ...
          lng: p.longitude,
          distanceKm,
          availableNow: p.isAvailableNow || hasActiveSession,
          planTier: p.planTier,
          isActive: p.isActive,
...
async function updateProfile(req: any, res: any) {
  ...
  const user = await prisma.user.update({
    ...
    data: {
      ...
      isAvailableNow: safeIsAvailableNow,
      isAvailableNowUpdatedAt:
        safeIsAvailableNow !== undefined ? new Date() : undefined,
    },
  });

  if (typeof attributeSlugs === "string") {
    const slugs = attributeSlugs
      .split(",")
      .map((v) => v.trim())
      .filter(Boolean);
    const attrs = slugs.length
      ? await prisma.profileAttribute.findMany({
          where: { slug: { in: slugs }, isActive: true },
        })
      : [];
    await prisma.profileAttributeOnProfile.deleteMany({
      where: { profileId: req.session.userId! },
    });
    if (attrs.length) {
      await prisma.profileAttributeOnProfile.createMany({
        data: attrs.map((a) => ({
