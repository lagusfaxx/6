...
async function main() {
  ...
  await prisma.establishmentReview.deleteMany({
    where: { establishment: { name: { startsWith: "Establecimiento" } } },
  });
  await prisma.establishment.deleteMany({
    where: { name: { startsWith: "Establecimiento" } },
  });
  ...
  const professionalCategories = await prisma.category.findMany({
    where: { kind: "PROFESSIONAL" },
  });
  const establishmentCategories = await prisma.category.findMany({
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    const professional = await prisma.user.create({
      data: {
        ...
        planTier: ["PLATINUM", "GOLD", "SILVER"][i % 3],
        moderationStatus: "APPROVED",
        isAvailableNow: i % 2 === 0,
        isAvailableNowUpdatedAt: new Date(),
        isOnline: i % 2 === 0,
  ...
  for (let i = 0; i < 5; i += 1) {
    const category =
      establishmentCategories[i % establishmentCategories.length];
  ...
  await prisma.profileAttribute.createMany({
    data: [
      { slug: "hombres", label: "Hombres", type: "GENDER_GROUP" },
      { slug: "mujeres", label: "Mujeres", type: "GENDER_GROUP" },
      { slug: "trans", label: "Trans", type: "GENDER_GROUP" },
      { slug: "maduras", label: "Maduras", type: "GENDER_GROUP" },
      { slug: "masajes", label: "Masajes", type: "SERVICE_TAG" },
      {
        slug: "despedidas-soltero",
        label: "Despedidas de soltero",
        type: "SERVICE_TAG",
      },
      { slug: "packs", label: "Packs", type: "SERVICE_TAG" },
      { slug: "video-llamadas", label: "Video llamadas", type: "SERVICE_TAG" },
      { slug: "destacada", label: "Destacada", type: "FEATURE" },
    ],
    skipDuplicates: true,
  });

  const hasTerms = await prisma.termsVersion.findFirst({
    where: { isActive: true },
  });
  if (!hasTerms) {
